...
#define LEN(a) (sizeof a / sizeof *a)
	...
	#define POPCNT32(x) tiifhe_util_popcount32(x)
	#define ROL64(x, r) tiifhe_util_rol64(x, r)
#endif
...
static void random_bytes(void *out, size_t len, tiifhe_Seed *seed);
...
static void
random_bytes(void *out, size_t len, tiifhe_Seed *seed)
{
	unsigned char *ptr, *s;
	size_t unused, i;
...
void
tiifhe_random_bytes(unsigned char *out, size_t len, tiifhe_Seed *seed)
{
...
void
tiifhe_random_cbd1_i8(int8_t *out, size_t len, tiifhe_Seed *seed)
{
	size_t i;
...
void
tiifhe_random_cbd1_i64(int64_t *out, size_t len, tiifhe_Seed *seed)
{
	size_t i;
...
void
tiifhe_random_cbd21_i8(int8_t *out, size_t len, tiifhe_Seed *seed)
{
	size_t i;
...
void
tiifhe_random_cbd21_i64(int64_t *out, size_t len, tiifhe_Seed *seed)
{
	size_t i;
...
void
tiifhe_random_seed(tiifhe_Seed *seed)
{
	#if TIIFHE_FIX_SEED
	static int add = 0;
	size_t i;

	for (i = 0; i < KECCAK_SEED_LEN; ++i)
		seed->value[i] = (i + add) & 0xff;
	add += 1;
	#else
...
void
tiifhe_random_uniform_i64(int64_t *out, size_t len, uint64_t mod, tiifhe_Seed *seed)
{

	tiifhe_random_uniform_u64((uint64_t *)out, len, mod, seed);
}

void
tiifhe_random_uniform_u64(uint64_t *out, size_t len, uint64_t mod, tiifhe_Seed *seed)
{
	uint64_t mask;
	size_t i;
